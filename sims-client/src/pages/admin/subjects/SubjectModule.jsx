...
const SubjectModule = () => {
  ...
  const categories = ['Science', 'Arts', 'Humanities', 'Technology', 'Language'];
  ...
  const handleAddSubject = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/subjects/', subjectToAdd, {
        headers: {
      ...
      setAlert({ message: 'Failed to add subject: ' + (error.response?.data?.message || error.message), type: 'error' });
    }
  };
  ...
  const handleUpdateSubject = async () => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/subjects/${editingId}`, updatedSubject, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
      ...
      setAlert({ message: 'Failed to update subject: ' + (error.response?.data?.message || error.message), type: 'error' });
    }
  };
