// src/pages/messages/MessageModule.jsx
...
function MessageModule() {
  ...
  const filteredMessages = useMemo(() => {
    return messages.filter(message => {
      // Filter by tab
      if (activeTab === 'inbox') {
        if (message.status === 'sent' && message.sender === 'admin@school.edu') return false; // Exclude sent by current user
        if (message.status === 'draft' || message.status === 'trash') return false; // Exclude drafts and trash
      }

      if (activeTab === 'sent' && (message.status !== 'sent' || message.sender !== 'admin@school.edu')) return false; // Only show sent by current user
      if (activeTab === 'drafts' && message.status !== 'draft') return false;

      // Trash logic: only show messages with status 'trash' and within the retention period
      if (activeTab === 'trash') {
        if (message.status !== 'trash') return false;
        if (message.deletedAt) {
          const days = getDaysSinceDeletion(message.deletedAt);
          if (days > TRASH_RETENTION_DAYS) return false; // Hide if past retention
        } else {
      ...
      if (activeTab === 'starred' && (!message.starred || message.status === 'trash')) return false;
  ...
  const unreadCount = useMemo(() => messages.filter(m => !m.read && m.status === 'sent' && m.sender !== 'admin@school.edu' && m.status !== 'trash').length, [messages]);
