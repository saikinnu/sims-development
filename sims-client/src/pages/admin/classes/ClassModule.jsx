...
const ClassModule = () => {
  ...
  const [newClass, setNewClass] = useState({
    ...
    grade: '',
    supervisor: '',
    teachers_details: [{ name: '', empId: '', subjects: '' }]
  });
  ...
  useEffect(() => {
    const results = classes.filter((cls) => {
      try {
        ...
        if (
          (cls.className?.toLowerCase() || '').includes(searchTermLower) ||
          (cls.grade?.toLowerCase() || '').includes(searchTermLower) ||
          (cls.supervisor?.toLowerCase() || '').includes(searchTermLower)
        ) {
          return true;
        }

        // Check teachers_details if it exists
        if (cls.teachers_details && Array.isArray(cls.teachers_details)) {
          return cls.teachers_details.some((teacher) => {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        ...
        // Transform the API response to match your frontend structure if needed
        const formattedClasses = response.data.map(cls => ({
          id: cls._id, // MongoDB uses _id by default
          className: cls.class_name,
          strength: cls.strength,
          grade: cls.grade,
          supervisor: cls.supervisor,
          teachers_details: cls.teachers_details || []
        }));
        ...
        setAlert({
          message: error.response?.data?.message || 'Failed to fetch classes. Please try again.',
          type: 'error'
        });
  ...
  const addTeacherField = () => {
    setNewClass(prev => ({
      ...prev,
      teachers_details: [...prev.teachers_details, { name: '', empId: '', subjects: '' }]
    }));
  };

  // Handle remove teacher field in form
  const removeTeacherField = (index) => {
    const updatedTeachers = [...newClass.teacteachers_detailshers];
    updatedTeachers.splice(index, 1);
    setNewClass(prev => ({
      ...prev,
      teachers_details: updatedTeachers
    }));
  };

  // Handle teacher input change
  const handleTeacherChange = (index, field, value) => {
    const updatedTeachers = [...newClass.teachers_details];
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate teachers (keep your existing validation)
    const validTeachers = newClass.teachers_details.filter(t => t.name && t.empId && t.subjects);
    if (validTeachers.length === 0 && newClass.teachers_details.length > 0 &&
      (newClass.teachers_details[0].name || newClass.teachers_details[0].empId || newClass.teachers_details[0].subjects)) {
      setAlert({ message: 'Please ensure all teacher fields are filled for each teacher, or remove empty teacher entries.', type: 'error' });
      return;
    }
    ...
    try {
      ...
      const classData = {
        ...
        teachers_details: validTeachers.map(t => ({
          ...t,
          subjects: typeof t.subjects === 'string' ?
      ...
      if (editingClassId) {
        ...
        setClasses(classes.map(cls =>
          cls.id === editingClassId ? response.data : cls
        ));
      ...
      setAlert({
        message: error.response?.data?.message || 'Failed to save class. Please try again.',
        type: 'error'
      });
    }
  };
  ...
  const handleEdit = (classId) => {
    ...
    setNewClass({
      ...
      teachers_details: classToEdit.teachers_details.length > 0
        ? classToEdit.teachers_details.map(t => ({ ...t, subjects: t.subjects.join(', ') }))
        : [{ name: '', empId: '', subjects: '' }]
    });
  ...
  const handleDelete = async (classId) => {
    if (window.confirm('Are you sure you want to delete this class?')) {
      try {
        ...
        setAlert({
          message: error.response?.data?.message || 'Failed to delete class. Please try again.',
          type: 'error'
        });
      }
    }
  ...
  const uniqueGrades = ['Grade 1', 'Grade 2', 'Grade 3', 'Grade 4', 'Grade 5'];
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-5">
                ...
                <div>
                  ...
                  {newClass.teachers_details.map((teacher, index) => (
                    <div key={index} className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3 items-center">
                      ...
                      <input
                        ...
                        required
                      />
                      {newClass.teachers_details.length > 1 && (
                        <button
                          type="button"
            ...
            <div className="p-6">
              ...
              <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                ...
                <div className="divide-y divide-gray-100">
                  {selectedClass.teachers_details.length > 0 ? (
                    selectedClass.teachers_details.map((teacher, index) => (
