...
const StudentModule = () => {
    ...
    if (window.confirm('Are you sure you want to delete this student record?')) {
      ...
      try {
        ...
        if (!token) {
          ...
          // Handle redirect to login or token refresh
          return;
        }
    ...
    const fetchStudents = async () => {
      try {
        ...
        const formattedStudents = response.data.map(student => ({
          ...
          email: student.email,
          class: student.class_id, // You might want to populate this properly
          contact: student.contact || '',
          parent: student.parent_id?.[0] || '', // Assuming single parent
          status: 'active', // Default status
          address: student.address,
          avatar: student.profile_image?.url || ''
        }));
        console.log('api data : ', formattedStudents);
  ...
  const handleAddStudent = async () => {
    ...
    try {
      const studentData = {
        ...
        full_name: newStudent.name,
        email: newStudent.email,
        admission_number: newStudent.rollNumber,
        // date_of_birth: newStudent.dob, // format: YYYY-MM-DD or new Date(newStudent.dob)
        // gender: newStudent.gender,
        address: newStudent.address,
        parent_id: newStudent.parent, // must be an existing Parent _id
        class_id: newStudent.class,     // must be an existing Class _id
        // blood_group: newStudent.bloodGroup,
        // medical_notes: newStudent.medicalNotes
      };
      ...
      //   // gender: newStudent.gender,
      //   address: '',
      //   parent_id: '', // must be an existing Parent _id
      //   class_id: '',     // must be an existing Class _id
      //   // blood_group: newStudent.bloodGroup,
      //   // medical_notes: newStudent.medicalNotes
      // });
    } catch (error) {
  ...
  const handleUpdateStudent = async () => {
    ...
    try {
      const updateData = {
        user_id: viewStudent.studentId,
        full_name: viewStudent.name,
        admission_number: viewStudent.rollNumber,
        email: viewStudent.email,
        class_id: viewStudent.class,
        address: viewStudent.address,
        parent_id: [viewStudent.parent], // Array as per your schema
        status: viewStudent.status
      };
      if (viewStudent.newPassword) {
        updateData.password = viewStudent.newPassword;
      }
      const response = await axios.put(
        `http://localhost:5000/api/students/${viewStudent.id}`,
        updateData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
