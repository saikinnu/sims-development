...
const AssignmentModule = () => {
  ...
  const [editingAssignment, setEditingAssignment] = useState(null); // New state to hold assignment being edited

  const [newAssignment, setNewAssignment] = useState({
    title: "",
    class: "", // Will store class _id
    subject: "", // Will now store subject _id
    dueDate: "",
    description: "",
  });
  ...
  const [classes, setClasses] = useState([]); // Stores fetched class objects {_id, name}
  const [subjects, setSubjects] = useState([]); // Stores fetched subject objects {_id, name}
  const [availableFilterClasses, setAvailableFilterClasses] = useState(["All"]);

  // Helper function to safely get the name from a populated object
  const getPopulatedName = (obj) => {
    // console.log("getPopulatedName received:", obj, "Type:", typeof obj); // Keep for debugging if needed
    if (!obj) {
      // console.log("getPopulatedName returning empty string for null/undefined obj");
      return '';
    }
    if (typeof obj === 'string') {
      // console.log("getPopulatedName returning string (ID):", obj);
      return obj; // It's an ObjectId string, return it as is
    }
    // If it's an object, try to find a name property
    const nameCandidate = obj.name || obj.class_name || obj.subject_name;
    if (typeof nameCandidate === 'string' && nameCandidate !== '') {
      // console.log("getPopulatedName returning extracted name:", nameCandidate);
      return nameCandidate;
    }
    // Fallback: If it's an object but no suitable name string was found,
    // this is where the error likely originates if not handled.
    console.error("getPopulatedName could not extract string name from object, returning empty string. Object:", obj);
    return ''; // Prevent React from trying to render the object
  };

  // Fetch assignments, classes, and subjects on component mount
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const token = JSON.parse(localStorage.getItem('authToken')); // Assuming token is stored
        ...
        const classNamesForFilter = ["All", ...classesResponse.data.map(cls => getPopulatedName(cls))].sort();
        setAvailableFilterClasses(classNamesForFilter);
        ...
        setAlert({ message: 'Data loaded successfully!', type: 'success' });
      } catch (error) {
        console.error("Error fetching initial data:", error);
        setAlert({ message: 'Failed to load data. Please check your server connection.', type: 'error' });
      }
    };
  ...
  // Handles input changes for both new and editing assignments
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Data being sent to backend (CREATE):", newAssignment);
    ...
    try {
      ...
      const newClassName = getPopulatedName(response.data.class);
      if (newClassName && !availableFilterClasses.includes(newClassName)) {
        setAvailableFilterClasses([...availableFilterClasses, newClassName].sort());
      }

    } catch (error) {
      console.error("Error creating assignment:", error);
      setAlert({ message: `Failed to create assignment: ${error.response?.data?.message || error.message}`, type: 'error' });
    }
  };

  // Function to handle updating an existing assignment
  const handleUpdateAssignment = async (e) => {
    e.preventDefault();

    if (!editingAssignment.title || !editingAssignment.class || !editingAssignment.subject || !editingAssignment.dueDate) {
      setAlert({ message: 'Please fill in all required fields (Title, Class, Subject, Due Date).', type: 'error' });
      return;
    }

    console.log("Data being sent to backend (UPDATE):", editingAssignment);

    try {
      const token = JSON.parse(localStorage.getItem('authToken'));
      const response = await axios.put(`http://localhost:5000/api/assignments/${editingAssignment._id}`, editingAssignment, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      setAssignments(assignments.map(assign =>
        assign._id === response.data._id ? response.data : assign
      ));
      ...
      setAlert({ message: `Failed to update assignment: ${error.response?.data?.message || error.message}`, type: 'error' });
    }
  };

  // Function to initiate editing an assignment
  const startEditAssignment = (assignment) => {
    // Ensure the class and subject are set as _id strings for the form
    setEditingAssignment({
      ...assignment,
      class: assignment.class?._id || assignment.class, // Set to ID if populated
      subject: assignment.subject?._id || assignment.subject // Set to ID if populated
    });
  ...
  const deleteAssignment = async (id) => {
    if (window.confirm('Are you sure you want to delete this assignment?')) {
      try {
        ...
        setAlert({ message: `Failed to delete assignment: ${error.response?.data?.message || error.message}`, type: 'error' });
      }
    }
  };
  ...
  const filteredAssignments = selectedClassFilter === "All"
    ? assignments
    : assignments.filter(assignment => getPopulatedName(assignment.class) === selectedClassFilter);
  ...
  return (
    <div className="px-0 sm:px-2 md:px-4 lg:p-6 flex flex-col gap-2 sm:gap-4 lg:gap-8">
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="col-span-1 md:col-span-1">
          ...
          <div className="relative">
            ...
            >
              {availableFilterClasses.map((clsName) => (
            ...
            {filteredAssignments.length > 0 ? (
              filteredAssignments.map((assignment) => (
                <tr key={assignment._id} className="transition-colors duration-200 hover:bg-indigo-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{getPopulatedName(assignment.class)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{getPopulatedName(assignment.subject)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{new Date(assignment.dueDate).toLocaleDateString()}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        onClick={() => startEditAssignment(assignment)} // Added onClick for edit
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {classes.map((cls) => (
                        <option key={cls._id} value={cls._id}>{getPopulatedName(cls)}</option>
                      ))}
                    </select>
                    <svg className="w-5 h-5 absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none text-gray-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                  </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {subjects.map((sub) => (
                        <option key={sub._id} value={sub._id}>{getPopulatedName(sub)}</option>
                      ))}
                    </select>
                    <svg className="w-5 h-5 absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none text-gray-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                  </div>
      ...
      {/* Edit Assignment Form (Modal) - NEW */}
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 md:p-8 w-full max-w-md shadow-2xl border border-gray-100">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-200">
              ...
              <button
                onClick={() => { setShowEditForm(false); setEditingAssignment(null); setAlert({ message: '', type: '' }); }}
                className="text-gray-500 hover:text-gray-700 p-1 rounded-full hover:bg-gray-100 transition-colors"
              >
                <X size={24} />
              </button>
            </div>
            <form onSubmit={handleUpdateAssignment}> {/* Submit to update handler */}
              <div className="space-y-5">
                <div>
                  <label htmlFor="editTitle" className="block text-sm font-medium text-gray-700 mb-1">Title <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    id="editTitle"
                    name="title"
                ...
                <div>
                  <label htmlFor="editClass" className="block text-sm font-medium text-gray-700 mb-1">Class <span className="text-red-500">*</span></label>
                  <div className="relative">
                    <select
                      id="editClass"
                      name="class"
                ...
                <div>
                  <label htmlFor="editSubject" className="block text-sm font-medium text-gray-700 mb-1">Subject <span className="text-red-500">*</span></label>
                  <div className="relative">
                    <select
                      id="editSubject"
                      name="subject"
                ...
                <div>
                  <label htmlFor="editDueDate" className="block text-sm font-medium text-gray-700 mb-1">Due Date <span className="text-red-500">*</span></label>
                  <input
                    type="date"
                    id="editDueDate"
                    name="dueDate"
                    value={editingAssignment.dueDate ? new Date(editingAssignment.dueDate).toISOString().split('T')[0] : ''} // Format date for input type="date"
                    onChange={handleInputChange}
                    className="w-full px-4 py-2.5 rounded-lg border border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-gray-900 placeholder-gray-400 text-base transition-colors"
                    required
                  />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-semibold text-gray-900">{getPopulatedName(selectedAssignment.class)}</p>
                  </div>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-semibold text-gray-900">{getPopulatedName(selectedAssignment.subject)}</p>
                  </div>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-semibold text-gray-900">{new Date(selectedAssignment.dueDate).toLocaleDateString()}</p>
                  </div>
                </div>
              </div>
