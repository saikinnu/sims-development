...
import SIMSLogo from '../../../assets/sims.png'; // Corrected path for parent's context

// Import specific Font Awesome icons from 'react-icons/fa'
import { FaBars, FaComments, FaBullhorn } from 'react-icons/fa'; // Added FaComments and FaBullhorn

const Navbar = ({ isMobileMenuOpen, setIsMobileMenuOpen }) => { // Accept props for mobile menu
  ...
  return (
    <div className='flex items-center justify-between p-4 bg-white shadow-sm dark:bg-gray-800'>
      ...
      <div className='flex items-center gap-4 sm:gap-6 ml-auto'> {/* ml-auto to push items to right */}
        {/* Messages Icon */}
        <div
        ...
        <div className="relative" ref={dropdownRef}>
          <button onClick={() => setShowProfileDropdown(!showProfileDropdown)} className="flex items-center gap-2">
            ...
            <img
              ...
              width={32}
              height={32}
              className="rounded-full w-8 h-8 object-cover" // Added w-8 h-8 object-cover for circularity
            />
          </button>
